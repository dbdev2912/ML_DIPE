...
export default (props) => {
    ...
    const { closeBox, tables, table, addFKtoTable } = props;
    ...
    const submit = () => {
        ...
        fetch(`/api/${ unique_string }/database/table/key/foreign`, {
            method: "POST",
            headers: {
                'Content-type': "application/json"
            },
            body: JSON.stringify({ key: data })
        }).then( res => res.json() ).then( ({ insertResult }) => {
            addFKtoTable( data );
        })

        /* SAVE FOREIGN KEY TO DATABASE AND MANIPULATE REAL TABLE */
        closeBox();
    }
